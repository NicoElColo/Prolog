...
suma_horas(Horas,Cantidad,Resul):-
    Aux is Horas+Cantidad,
    Aux < 24,
    Resul is Aux.
suma_horas(Horas,Cantidad,Resul):-
    Aux is Horas+Cantidad,
...
camino_aux(Origen,Destino,Llegada,HorasAcum,Visitados,[[Origen,Posta,Salida,Llego]|Resul]):-
    viaje(Origen,Posta,Salida,Horas),
    Posta \= Destino,
    Cond is Salida - HorasAcum,
    Cond =< 2, Cond >= 0,
    suma_horas(HorasAcum,Cond,Espera),
...
camino(Origen,Destino,Llegada,[[Origen,Posta,Salida,Llego]|Tramos]):-
    viaje(Origen,Posta,Salida,Horas),
    suma_horas(Salida,Horas,Llego),
    camino_aux(Posta,Destino,Llegada,Llego,[Posta|[Origen]],Tramos).